...
export default function NgoNotificationsPage() {
 ...
 try {
 setLoading(true);
 const res = await fetch("/api/ngo/notifications");
 ...
 try {
 await fetch("/api/ngo/notifications", { method: "PATCH" });
 ...
 try {
 await fetch("/api/ngo/notifications", { method: "DELETE" });
 setNotifications([]);
 ...
 <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 sm:mb-10">
 <motion.div
 ...
 className="group flex-1 md:flex-none min-w-[140px] px-6 py-3 bg-white border border-slate-100 text-slate-950 font-black rounded-2xl flex items-center justify-center gap-3 hover:bg-orange-50 hover:text-orange-600 transition-all shadow-sm active:scale-95 text-[10px] uppercase tracking-widest"
 >
 ...
 className="group flex-1 md:flex-none min-w-[140px] px-6 py-3 bg-slate-950 text-white font-black rounded-2xl flex items-center justify-center gap-3 hover:bg-red-600 transition-all shadow-xl active:scale-95 text-[10px] uppercase tracking-widest"
 >
 ...
 className={`group p-4 sm:p-6 rounded-[2rem] border flex items-start gap-3 sm:gap-5 transition-all cursor-pointer min-w-0 ${
 !notif.isRead
 ...
 <h4 className="font-black text-sm sm:text-base tracking-tight break-words whitespace-normal leading-tight">{notif.title}</h4>
 </div>
 <p className="text-sm font-bold text-slate-500 leading-relaxed break-words">{notif.message}</p>
 ...
 <p className="text-slate-400 text-sm font-bold mt-2 max-w-sm">When donor updates or logistics actions happen, you&apos;ll see them here.</p>
 </div>
 )}
 </AnimatePresence>
 ...
 onClick={(e) => {
 e.preventDefault();
 if (currentPage < Math.ceil(notifications.length / itemsPerPage)) setCurrentPage(currentPage + 1);
 }}
