    ...
    return (
        <header>
            <nav
                ...
                className="fixed left-0 w-full z-[60] px-3 top-2 sm:top-4">
                <div className="mx-auto max-w-7xl rounded-[1.5rem] sm:rounded-[2.5rem] border border-orange-100 bg-white/80 backdrop-blur-2xl px-4 sm:px-8 py-3 sm:py-4 flex items-center justify-between shadow-[0_10px_40px_rgba(0,0,0,0.04)] transition-all">
                    ...
                    <div className="flex items-center gap-2 sm:gap-4">
                        ...
                        <div className="flex lg:hidden items-center gap-2 sm:gap-3">
                            ...
                            <button
                                ...
                                className="relative z-20 m-0 block cursor-pointer p-2 sm:p-2.5 text-slate-900 bg-slate-50 border border-slate-100 rounded-xl sm:rounded-2xl">
                                {menuState ? <X className="size-5 sm:size-6 transition-all" /> : <Equal className="size-5 sm:size-6 transition-all" />}
                            </button>
                        </div>
                    </div>

                    <div className={cn(
                        "bg-white lg:hidden absolute top-[100%] left-0 right-0 p-6 sm:p-8 shadow-2xl rounded-b-[2rem] border-x border-b border-orange-100 transition-all duration-300 ease-in-out z-50",
                        menuState ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 -translate-y-4 pointer-events-none"
                    )}>
                        <div className="w-full">
                            <ul className="flex flex-col items-center gap-5 sm:gap-6 text-[10px] sm:text-[11px] font-black uppercase tracking-[0.2em] sm:tracking-[0.25em] text-slate-400 w-full text-center">
                                {menuItems.map((item, index) => (
                                ...
                                <li className="w-full pt-2">
                                    <Button
                                        asChild
                                        size="sm"
                                        className="w-full px-8 py-5 bg-slate-950 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-orange-600 transition-all active:scale-95 flex items-center justify-center gap-2">
                                        <Link href="/register">
